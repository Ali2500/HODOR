...
def build_lr_scheduler(optimizer, scheduler_type):

    if scheduler_type == "none":
        return None

    elif scheduler_type == "warmup":
        sched_cfg = cfg.TRAINING.LR_SCHEDULERS.WARMUP

        return WarmupLR(
            optimizer,
            warmup_iters=sched_cfg.WARMUP_ITERS,
            warmup_lr_init=sched_cfg.WARMUP_LR_INIT
        )

    elif scheduler_type == "step":
        sched_cfg = cfg.TRAINING.LR_SCHEDULERS.STEP

        return StepLR(
            optimizer,
            step_iters=sched_cfg.STEPS,
            decay_rate=sched_cfg.DECAY_RATES,
            warmup_iters=sched_cfg.WARMUP_ITERS,
            warmup_lr_init=sched_cfg.WARMUP_LR_INIT
        )

    else:
        raise NotImplementedError(f"Invalid LR scheduler type given: '{scheduler_type}'")
