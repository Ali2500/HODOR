...
def build_swin_fpn(variant: str, pretrained: bool):
    ...
    swin = SwinTransformer(**configs[variant])
    ...
    if pretrained:
        ckpt_path = osp.join(Paths.pretrained_backbones_dir(), f"swin_{variant}_pretrained.pth")
        assert osp.exists(ckpt_path), f"Checkpoint not found at '{ckpt_path}'"

        ckpt = torch.load(ckpt_path)["state_dict"]
        ckpt_bb = {k[len("backbone."):]: v for k, v in ckpt.items() if k.startswith("backbone")}
        swin.load_state_dict(ckpt_bb, strict=True)

        ckpt_neck = {k[len("neck."):]: v for k, v in ckpt.items() if k.startswith("neck")}
        fpn.load_state_dict(ckpt_neck, strict=True)

    return nn.Sequential(swin, fpn)


def _test():
    model = build_swin_fpn(variant="base", pretrained=True).cuda()
